import Day03.gearSymbols
import Day03.input
import Day03.parseValues
import Day03.partNumbers
import Day03.symbols
import Day03.filterFor
import Day03.gearRatios

fun main() {
    val valueMap = parseValues(input)
    val partNumbers = partNumbers(valueMap)
    val symbols = symbols(valueMap)

    val answer1 = partNumbers.filterFor(symbols).sumOf { it.number }
    println(answer1) // 532445

    val answer2 = partNumbers.gearRatios(symbols.gearSymbols()).sum()
    println(answer2) // 79842967
}

object Day03 {

    data class Position(val x: Int, val y: Int) {
        fun isBefore(other: Position): Boolean = this.y == other.y && this.x + 1 == other.x
    }

    data class PartNumber(val chars: List<Char>, val positions: List<Position>) {

        val number = chars.joinToString("").toInt()

        fun isAdjacentTo(position: Position): Boolean {
            val xRange = positions.first().x - 1..positions.last().x + 1
            val yRange = positions.first().y - 1..positions.first().y + 1
    ...
    fun symbols(values: Map<Position, Char>): List<Symbol> =
        values.entries.mapNotNull { (position, c) ->
            if (c.isDigit()) null else Symbol(c, position)
        }

    fun List<PartNumber>.filterFor(symbols: List<Symbol>): List<PartNumber> =
        filter { partNumber ->
            symbols.any { symbol ->
                partNumber.isAdjacentTo(symbol.position)
            }
        }

    fun List<PartNumber>.gearRatios(gearSymbols: List<Symbol>): List<Int> =
        gearSymbols.mapNotNull { symbol ->
            val partNumbers = filter { it.isAdjacentTo(symbol.position) }
            if (partNumbers.size == 2) partNumbers[0].number * partNumbers[1].number
            else null
        }

    fun List<Symbol>.gearSymbols() = filter { it.symbol == '*' }

    fun partNumbers(values: Map<Position, Char>): List<PartNumber> =
        values.entries.fold(emptyList()) { acc, (position, c) ->
            when {
                !c.isDigit() -> acc
                acc.isEmpty() -> listOf(PartNumber(listOf(c), listOf(position)))
                else -> {
                    val lastPartNumber = acc.last()
                    val lastPosition = lastPartNumber.positions.last()
                    if (lastPosition.isBefore(position))
                        acc.dropLast(1) + PartNumber(lastPartNumber.chars + c, lastPartNumber.positions + position)
                    else acc + PartNumber(listOf(c), listOf(position))
                }
            }
        }

    fun parseValues(s: String): Map<Position, Char> =
