import Day02.CubeColour.blue
import Day02.CubeColour.green
import Day02.CubeColour.red
import Day02.input
import Day02.parseGames

fun main() {
    val games = parseGames(input)

    val answer1 = games.filter { it.isPossible() }.sumOf { it.id }
    println(answer1) // 2683

    val answer2 = games.sumOf { game -> game.minCubes().map { it.value }.reduce { a, b -> a * b } }
    println(answer2) // 49710
}
...
object Day02 {
    ...
    object Bag {
        private val contents = mapOf(red to 12, green to 13, blue to 14,)
        fun isPossible(colour: CubeColour, count: Int): Boolean = contents.getValue(colour) >= count
    }

    data class Game(val id: Int, val reveals: List<Map<CubeColour, Int>>) {

        fun isPossible(): Boolean =
            reveals.all { reveal ->
                reveal.entries.all { (colour, n) ->
                    Bag.isPossible(colour, n)
                }
            }

        fun minCubes(): Map<CubeColour, Int> =
            reveals.fold(mapOf(red to 0, green to 0, blue to 0)) { acc, next ->
                acc.map { (colour, n) ->
                    val revealN = next.getOrDefault(colour, 0)
                    colour to if (revealN > n) revealN else n
                }.toMap()
            }
    }

    fun parseGames(s: String): List<Game> = s.lines().map { parseGame(it) }

    private fun parseGame(line: String): Game {
        val (gamePart, revealsPart) = line.split(":")
        return Game(
            id = gamePart.split(" ").last().toInt(),
            reveals = revealsPart.trim().split("; ")
                .map { it.split(", ") }
                .map { colourCountParts ->
                    colourCountParts.associate {
                        val (n, colour) = it.split(" ")
                        CubeColour.valueOf(colour) to n.toInt()
                    }
                }
        )
    }
